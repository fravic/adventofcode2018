...
func main() {
	...
	mostAsleepGuard, mostAsleepMinute := findMostFrequentlyAsleepGuardAndMinute(guards)
...
func findMostFrequentlyAsleepGuardAndMinute(guards map[int]*guard) (*guard, int) {
	var maxGuard *guard
	maxMinute := 0
	for _, guard := range guards {
		for minute, count := range guard.asleepCountByMinute {
			if maxGuard == nil || count > maxGuard.asleepCountByMinute[maxMinute] {
				maxGuard = guard
				maxMinute = minute
			}
		}
